...
namespace NexgenDragon
{
	...
	public class Crc32Algorithm : HashAlgorithm
	{
        ...
        public new byte[] ComputeHash(Stream inputStream)
        {
            ...
            int bytesRead;
            while ((bytesRead = inputStream.Read(_buffer, 0, 4096)) > 0)
            {
                HashCore(_buffer, 0, bytesRead);
            }
            return HashFinal();
        }
        
        public uint ComputeCrcValue(string inputPath)
        {
	        try
	        {
		        using (var fs = new FileStream(inputPath, FileMode.Open, FileAccess.Read, FileShare.Read))
		        {
			        ...
			        int bytesRead;
			        while ((bytesRead = fs.Read(_buffer, 0, 4096)) > 0)
			        {
				        HashCore(_buffer, 0, bytesRead);
			        }
			        return _currentCrc;
	        ...
	        {
		        NLogger.Error("ComputeCrcValue Error. inputPath={0}, Exception={1}", inputPath, e.ToString());
		        return 0;
	        }
        }
	}
